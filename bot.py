...
async def processar_grupo(client, nome_conta, grupo, data_inicio, data_fim, limite):
    nome_arquivo = f"{grupo.name.replace(' ', '_')}_todos_usuarios.csv"
    ja_salvos = ler_existente(nome_arquivo)
    dados = []
    contador = 0

    async for user in client.iter_participants(grupo):
        try:
            ...
            chave_unica = (link, str(user_id))
            if chave_unica in ja_salvos:
                continue

            dados.append([nome, username, link, user_id, access_hash, grupo.name, grupo.id])
            contador += 1
            print(f"⏳ [{nome_conta}] Coletando {grupo.name}... ({contador} encontrados)", end="\r")
    ...
    print(f"\n\n✅ [{nome_conta}] Extração finalizada. Total: {contador} usuários.")
